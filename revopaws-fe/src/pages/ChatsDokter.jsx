...
import ChatBubbleDokter from "../components/ChatBubbleDokter";
...
const ChatsDokter = () => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible" && document.hasFocus()) {
        if (messages.some((msg) => msg.role === "user" && msg.status !== "read")) {
          markAsRead();
        }
      }
    };
  ...
  const checkRoomAndSessions = async () => {
    ...
    try {
      ...
      if (roomData.configuration.doctor_sessions !== localStorage.getItem('doctor_sessions_' + id)) {
        Swal.fire({
          title: "Error",
  ...
  const markAsRead = async () => {
    const findUserRole = messages.filter(item => item.role === "user")
    ...
    if (file.file !== null) {
      ...
      try {
        ...
        const data = DataSentChat({
          user_id: "",
          doctor_id: id,
          sessions: localStorage.getItem('doctor_sessions_' + id),
          doctor_name: "Dr. Dedi Sudayat",
          role: "doctor",
          message_type: "file",
          ...
          <div ref={chatContainerRef} className="w-[70%] gap-5 overflow-y-auto hide-scrollbar relative flex flex-col h-[100vh] bg-gray-900">
            <div className="flex flex-col gap-5 px-4 pt-5 h-auto pb-32 w-full">
              {messages.map((msg, index) => (
                <ChatBubbleDokter key={index} message={msg.message} messType={msg.message_type} fileUrl={msg.file_url} doctorName={msg.doctor_name} status={msg.status} timestamp={msg.timestamp.slice(0, 21)} role={msg.role} />
              ))}
            </div>
            <Formik
              ...
              {({ isSubmitting }) => (
                <Form className="flex items-center fixed bottom-0 w-[70%] px-3 py-4 bg-gray-700 dark:bg-gray-700">
                  ...
                  <button
                    ...
                    className="inline-flex justify-center p-2 text-white-500 bg-blue-500 rounded-full cursor-pointer hover:bg-blue-600 dark:hover:bg-gray-600"
                  >
                    <svg
